...
/// A mixin that gives easy access to helpful data and methods to make your widget more responsive and adaptive.
mixin  Adaptive {
  ...
  EdgeInsets get mediaQueryViewPadding => MediaQuery.of(context).viewPadding;

  /// similar to MediaQuery.of(_context).viewInsets
  EdgeInsets get mediaQueryViewInsets => MediaQuery.of(context).viewInsets;
  ...
  bool get mediumDesktop => (mediaQueryShortestSide >= 1280);
  ...
  T phone<T>({required T small, T? medium, T? large}) {
    if (largePhone && large != null) {
      return large;
    } else if (mediumPhone && medium != null && !smallPhone) {
      return medium;
    } else {
      return small;
    }
